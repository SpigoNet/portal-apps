    ...
    <div class="py-12">
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <form action="{{ isset($materia) ? route('ant.admin.materias.update', $materia->id) : route('ant.admin.materias.store') }}" method="POST">
                    @csrf
                    @if(isset($materia)) @method('PUT') @endif

                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700">Nome da Disciplina</label>
                        <input type="text" name="nome" value="{{ $materia->nome ?? old('nome') }}" required placeholder="Ex: Laboratório de Banco de Dados"
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" name="nome_curto" value="{{ $materia->nome_curto ?? old('nome_curto') }}" required placeholder="Ex: LBD"
                    ...
                    <div class="flex justify-end">
                        <a href="{{ route('ant.admin.materias.index') }}" class="bg-gray-200 text-gray-700 px-4 py-2 rounded mr-2 hover:bg-gray-300">Cancelar</a>
