<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Área de Notas e Trabalhos') }} - <span class="text-indigo-600">{{ $semestreAtual }}</span>
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200 flex justify-between items-center">
                    <div>
                        <h3 class="text-2xl font-bold text-gray-800">Olá, {{ explode(' ', $aluno->nome)[0] }}!</h3>
                        <p class="text-gray-500">RA: {{ $aluno->ra }}</p>
                    </div>
                    <div class="text-right hidden sm:block">
                        <span class="block text-sm text-gray-400">Disciplinas Matriculadas</span>
                        <span class="text-xl font-bold">{{ $materias->count() }}</span>
                    </div>
                </div>
            </div>
                ...
                @foreach($materias as $materia)
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-100">
                        <div class="bg-gray-50 px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                            ...
                            <span class="text-xs font-mono text-gray-400 bg-white px-2 py-1 rounded border">{{ $materia->nome_curto }}</span>
                        </div>

                        <div class="p-6">
                            @if($materia->trabalhos->isEmpty())
                                <p class="text-gray-400 text-sm italic">Nenhum trabalho ou prova agendado por enquanto.</p>
                            @else
                                <div class="overflow-x-auto">
                                    <table class="min-w-full divide-y divide-gray-200">
                                        <thead>
                                        <tr>
                                            <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Atividade</th>
                                            <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tipo</th>
                                            <th class="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Prazo</th>
                                            <th class="px-3 py-2 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th>
                                            <th class="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Ação</th>
                                        </tr>
                                        </thead>
                                        ...
                                        @foreach($materia->trabalhos as $trabalho)
                                            ...
                                            <tr class="{{ $atrasado ? 'bg-red-50' : '' }}">
                                                ...
                                                <td class="px-3 py-4 whitespace-nowrap">
                                                    <div class="text-sm font-medium text-gray-900">{{ $trabalho->nome }}</div>
                                                    <div class="text-xs text-gray-500 truncate w-64" title="{{ $trabalho->descricao }}">
                                                        {{ $trabalho->descricao }}
                                                    </div>
                                                </td>
                                                ...
                                                <td class="px-3 py-4 whitespace-nowrap">
                                                    <div class="text-sm {{ $atrasado ? 'text-red-600 font-bold' : 'text-gray-900' }}">
                                                        {{ $prazo->format('d/m/Y') }}
                                                    </div>
                                                ...
                                                <td class="px-3 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                    @if($isProva)
                                                        ...
                                                        <a href="{{ route('ant.prova.resultado', $trabalho->id) }}" class="text-purple-600 hover:text-purple-900 font-bold hover:underline">
                                                            Ver Prova / Resultado
                                                        </a>

                                                    @else
                                                        <a href="{{ route('ant.trabalhos.show', $trabalho->id) }}" class="text-indigo-600 hover:text-indigo-900 font-bold hover:underline">
                                                            @if($entrega)
                                                                Ver Detalhes
                                                            @elseif($atrasado)
                                                                Entregar com Atraso
