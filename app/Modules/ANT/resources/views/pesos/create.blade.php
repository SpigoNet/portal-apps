                    ...
                    <div class="bg-white shadow sm:rounded-lg p-6 sticky top-6">
                        ...
                        <form action="{{ route('ant.pesos.store') }}" method="POST">
                            ...
                            <div class="mb-4">
                                ...
                                <select name="materia_id" required class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                            ...
                            <div class="mb-6">
                                ...
                                <input type="number" name="valor" step="0.5" min="0" max="100" placeholder="Ex: 10.0" required
                    ...
                    @forelse($materias as $materia)
                        <div class="bg-white shadow sm:rounded-lg overflow-hidden">
                            <div class="bg-gray-50 px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                                ...
                                <span class="text-xs font-mono bg-white border px-2 py-1 rounded">{{ $materia->nome_curto }}</span>
                            </div>
                            ...
                            <div class="p-6">
                                @if(isset($pesosExistentes[$materia->id]) && $pesosExistentes[$materia->id]->isNotEmpty())
                                    <table class="min-w-full">
                                        ...
                                        @foreach($pesosExistentes[$materia->id] as $peso)
                                            ...
                                            <tr>
                                                ...
                                                <td class="py-3 text-right">
                                                    <form action="{{ route('ant.pesos.destroy', $peso->id) }}" method="POST" onsubmit="return confirm('Tem certeza? Isso pode afetar trabalhos já criados.');">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="text-red-500 hover:text-red-700 text-xs font-bold uppercase">Excluir</button>
                                                    </form>
                                                </td>
                                            </tr>
                                        @endforeach
                                        ...
                                        <tr class="bg-gray-50">
                                            <td class="py-2 pl-2 text-sm font-bold text-gray-700">Total Distribuído:</td>
                                    ...
                                    @if($soma != 10 && $soma != 100)
                                        <p class="text-xs text-yellow-600 mt-2">⚠️ Atenção: A soma dos pesos geralmente deve ser 10 ou 100.</p>
                                    @endif

                                @else
                                    <p class="text-gray-400 italic text-sm text-center py-4">Nenhum grupo de notas definido ainda.</p>
                                @endif
                            </div>
                        </div>
                    @empty
