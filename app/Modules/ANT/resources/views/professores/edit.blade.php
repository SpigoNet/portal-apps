<x-ANT::layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            ...
            <a href="{{ route('ant.professor.trabalho', $trabalho->id) }}" class="text-gray-500 hover:text-gray-900">{{ $trabalho->nome }}</a>
            <span class="text-gray-400 mx-2">/</span>
            Editar Trabalho
        </h2>
    </x-slot>
        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form action="{{ route('ant.professor.trabalho.update', $trabalho->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                            ...
                            <div>
                                ...
                                <select name="materia_id" id="materia_id" onchange="filtrarPesos()" required
                                    ...
                                    @foreach($materias as $materia)
                                        <option value="{{ $materia->id }}" @selected(old('materia_id', $trabalho->materia_id) == $materia->id)>{{ $materia->nome }}</option>
                                    @endforeach
                                </select>
                            </div>
                                    ...
                                    @foreach($tipos as $tipo)
                                        <label class="flex items-start gap-2 cursor-pointer hover:bg-white rounded p-1 transition-colors">
                                            <input type="checkbox" name="tipos_ids[]" value="{{ $tipo->id }}"
                                                   @checked(in_array($tipo->id, old('tipos_ids', $trabalho->tipos_trabalho_ids ?? [])))
                            ...
                            <div class="md:col-span-2">
                                ...
                                <select name="peso_id" id="peso_id" required
                                    ...
                                    @foreach($pesos as $peso)
                                        <option value="{{ $peso->id }}" data-materia="{{ $peso->materia_id }}"
                                                @selected(old('peso_id', $trabalho->peso_id) == $peso->id)>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="descricao" rows="5" required placeholder="Descreva o que o aluno deve fazer..."
                                      class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('descricao', $trabalho->descricao) }}</textarea>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div>
                                <label class="block text-sm font-medium text-gray-700">Máximo de Alunos (Grupo)</label>
                                <input type="number" name="maximo_alunos" min="1" max="10"
                            ...
                            <div class="bg-purple-50 p-3 rounded border border-purple-200">
                                ...
                                <textarea name="dicas_correcao" rows="2" placeholder="Ex: O aluno deve usar INNER JOIN. Descontar nota se não formatar o código."
                                          class="mt-1 block w-full rounded-md border-purple-300 shadow-sm focus:border-purple-500 focus:ring-purple-500 text-sm">{{ old('dicas_correcao', $trabalho->dicas_correcao) }}</textarea>
                        ...
                        <div class="flex justify-end border-t border-gray-100 pt-4">
                            <a href="{{ route('ant.professor.trabalho', $trabalho->id) }}" class="bg-gray-200 text-gray-700 px-4 py-2 rounded mr-2 hover:bg-gray-300">Cancelar</a>
    ...
    <script>
        ...
        // Run on page load to show the correct peso options for the pre-selected materia
