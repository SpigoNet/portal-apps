@php
    $portalApp = (isset($app) && $app instanceof \App\Models\PortalApp)
        ? $app
        : ($portalApp ?? new \App\Models\PortalApp());
@endphp
...
<div class="grid grid-cols-1 gap-6">
    ...
    <div>
        ...
        <input id="title" name="title" type="text" value="{{ old('title', $portalApp->title ?? '') }}" required
    ...
    <div>
        ...
        <textarea id="description" name="description" rows="3" required
            class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('description', $portalApp->description ?? '') }}</textarea>
    </div>

    <!-- Link Inicial -->
    <div>
        <label for="start_link" class="block font-medium text-sm text-gray-700">Link Inicial (Ex: /todo-app)</label>
        <input id="start_link" name="start_link" type="text" value="{{ old('start_link', $portalApp->start_link ?? '') }}"
            required
            class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
    </div>
    ...
    <div>
        ...
        <select id="visibility" name="visibility" required
            class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
            <option value="public" @selected(old('visibility', $portalApp->visibility ?? '') == 'public')>Público (todos veem)
            </option>
            <option value="private" @selected(old('visibility', $portalApp->visibility ?? '') == 'private')>Privado (logados
                veem)</option>
            <option value="specific" @selected(old('visibility', $portalApp->visibility ?? '') == 'specific')>Específico
                (usuários selecionados)</option>
        </select>
    </div>

    <!-- Usuários (para visibilidade específica) -->
    <div id="users-selection" class="{{ old('visibility', $portalApp->visibility ?? '') == 'specific' ? '' : 'hidden' }}">
        ...
        <select id="users" name="users[]" multiple
            ...
            @foreach($users as $user)
                <option value="{{ $user->id }}" @selected(in_array($user->id, old('users', $portalApp->exists ? $portalApp->users->pluck('id')->toArray() : [])))>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input id="pwa_short_name" name="pwa_short_name" type="text" maxlength="30"
                    value="{{ old('pwa_short_name', $portalApp->pwa_short_name ?? '') }}"
                    class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
            ...
            <div>
                ...
                <input id="pwa_scope" name="pwa_scope" type="text" value="{{ old('pwa_scope', $portalApp->pwa_scope ?? '') }}"
                    class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
            </div>
            ...
            <div>
                ...
                <input id="pwa_background_color" name="pwa_background_color" type="color"
                    value="{{ old('pwa_background_color', $portalApp->pwa_background_color ?? '#1a1b26') }}"
            ...
            <div>
                ...
                <input id="pwa_theme_color" name="pwa_theme_color" type="color"
                    value="{{ old('pwa_theme_color', $portalApp->pwa_theme_color ?? '#ccf381') }}"
                    class="block mt-1 w-full h-10 rounded-md shadow-sm border-gray-300">
            </div>
            ...
            <div>
                ...
                <select id="pwa_display" name="pwa_display"
                    class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                    <option value="standalone" @selected(old('pwa_display', $portalApp->pwa_display ?? '') == 'standalone')>
                        Standalone (App Nativo)</option>
                    <option value="fullscreen" @selected(old('pwa_display', $portalApp->pwa_display ?? '') == 'fullscreen')>
                        Fullscreen (Tela Cheia)</option>
                    <option value="minimal-ui" @selected(old('pwa_display', $portalApp->pwa_display ?? '') == 'minimal-ui')>
                        Minimal UI</option>
                    <option value="browser" @selected(old('pwa_display', $portalApp->pwa_display ?? '') == 'browser')>Browser
                        (Navegador)</option>
                </select>
            </div>
            ...
            <div>
                ...
                <select id="pwa_orientation" name="pwa_orientation"
                    class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                    <option value="any" @selected(old('pwa_orientation', $portalApp->pwa_orientation ?? '') == 'any')>Qualquer
                    </option>
                    <option value="portrait" @selected(old('pwa_orientation', $portalApp->pwa_orientation ?? '') == 'portrait')>
                        Retrato (Vertical)</option>
                    <option value="landscape" @selected(old('pwa_orientation', $portalApp->pwa_orientation ?? '') == 'landscape')>Paisagem (Horizontal)</option>
                    <option value="natural" @selected(old('pwa_orientation', $portalApp->pwa_orientation ?? '') == 'natural')>
                        Natural</option>
                </select>
            </div>
            ...
            <div class="col-span-full">
                ...
                <input type="hidden" name="icon" id="iconPathInput" value="{{ old('icon', $portalApp->icon ?? '') }}">
                ...
                <div
                    ...
                    @forelse($icons as $iconFile)
                        ...
                        <div class="icon-option cursor-pointer p-1 border-2 rounded-lg transition hover:border-indigo-400 @if(old('icon', $portalApp->icon ?? '') == $iconUrl) border-indigo-600 bg-indigo-50 @else border-transparent @endif"
