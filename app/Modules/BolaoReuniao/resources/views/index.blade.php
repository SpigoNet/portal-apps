...
<div class="max-w-2xl w-full text-center space-y-8">
    @if(!$activeMeeting)
        <div class="glass p-8 rounded-3xl shadow-2xl">
            @auth
                <h1 class="text-4xl font-bold mb-6 bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-emerald-400">Novo Bolão</h1>
                <form action="{{ route('bolao.start') }}" method="POST" class="space-y-4">
                    ...
                    <button type="submit" class="w-full bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-400 text-white font-bold py-3 rounded-xl transition-all shadow-lg shadow-blue-500/20">
                        Começar Reunião
                    </button>
                </form>
            @else
                <h1 class="text-4xl font-bold mb-4 bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-emerald-400 text-italic">Bolão Fatec</h1>
    ...
    @else
        <div class="glass p-8 rounded-3xl shadow-2xl space-y-8">
            ...
            <div class="pt-4 border-t border-slate-800/50">
                <h1 class="text-3xl font-bold mb-1 bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-emerald-400">{{ $activeMeeting->name }}</h1>
            ...
            <div class="space-y-4">
                ...
                <form action="{{ route('bolao.end', $activeMeeting->id) }}" method="POST">
                    @csrf
                    <button type="submit" class="w-full bg-gradient-to-r from-red-600 to-red-500 hover:from-red-500 hover:to-red-400 text-white font-bold py-3 rounded-xl transition-all shadow-lg shadow-red-500/20">
                        Encerrar Reunião
                    </button>
                </form>
                @endauth
