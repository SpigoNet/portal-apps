        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                <form action="{{ route('mundos-de-mim.admin.ai-providers.update', $provider->id) }}" method="POST" class="grid grid-cols-1 gap-6">
                    ...
                    <div><label class="block text-sm font-medium text-gray-700">API Key</label><input type="text" name="api_key" value="{{ old('api_key', $provider->api_key) }}" class="mt-1 block w-full rounded-md border-gray-300"></div>
